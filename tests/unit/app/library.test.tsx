...
vi.mock("@/components/library/LibrarySkeleton", () => ({
  LibrarySkeleton: vi.fn((): React.ReactElement => <div data-testid="library-skeleton" />),
}));

vi.mock("@/components/dashboard/TestLibrary", () => ({
  TestLibrary: vi.fn((): React.ReactElement => <div data-testid="test-library" />),
}));

vi.mock("next/link", () => ({
  default: ({ children, href }: { children: React.ReactNode; href: string }): React.ReactElement => <a href={href}>{children}</a>,
}));
...
describe("LibraryPage", () => {
  ...
  beforeEach((): void => {
    vi.clearAllMocks();
    vi.mocked(useAuth).mockReturnValue({ user: { id: mockUserId } } as unknown as ReturnType<typeof useAuth>);
    vi.mocked(useEffectiveUserId).mockReturnValue(mockUserId);
    vi.mocked(useInitializeDatabase).mockReturnValue({ isInitialized: true, error: null } as unknown as ReturnType<typeof useInitializeDatabase>);
    vi.mocked(useQuizzes).mockReturnValue({ quizzes: [] as Quiz[], isLoading: false, error: null } as unknown as ReturnType<typeof useQuizzes>);
  });

  it("renders the loading skeleton when not initialized", () => {
    vi.mocked(useInitializeDatabase).mockReturnValue({ isInitialized: false, error: null } as unknown as ReturnType<typeof useInitializeDatabase>);
    render(<LibraryPage />);
    expect(screen.getByTestId("library-skeleton")).toBeDefined();
  });

  it("renders the loading skeleton when quizzes are loading", () => {
    vi.mocked(useQuizzes).mockReturnValue({ quizzes: [] as Quiz[], isLoading: true, error: null } as unknown as ReturnType<typeof useQuizzes>);
    render(<LibraryPage />);
    expect(screen.getByTestId("library-skeleton")).toBeDefined();
  });
  ...
  it("renders the test library when data is loaded", () => {
    ...
    vi.mocked(useQuizzes).mockReturnValue({ quizzes: mockQuizzes, isLoading: false, error: null } as unknown as ReturnType<typeof useQuizzes>);
