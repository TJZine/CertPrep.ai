...
const { quizzesData, resultsData, dbMock } = vi.hoisted(() => {
  ...
  const resultsWhere = vi.fn().mockReturnValue({
    equals: vi.fn().mockImplementation((userId: string) => ({
      toArray: vi.fn().mockImplementation(async () => resultsData.filter((result) => result.user_id === userId)),
    })),
  });
...
vi.mock('@/lib/utils', () => ({
  ...
  calculatePercentage: (correct: number, total: number): number => (total === 0 ? 0 : Math.round((correct / total) * 100)),
}));
...
describe('getOverallStats per-user scoping', () => {
  ...
  it('only counts quizzes and results for the requested user', async () => {
    ...
    expect(stats.weakestCategories).toEqual(
      expect.arrayContaining([expect.objectContaining({ category: 'Networking' })]),
    );
    expect(stats.weakestCategories.some((entry) => entry.category === 'Security')).toBe(false);
  });
});
