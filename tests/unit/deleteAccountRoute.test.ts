...
vi.mock('next/headers', () => ({
  cookies: vi.fn(async () => ({
    ...
    get: (name: string): CookieRecord | undefined => cookieJar.find((c) => c.name === name),
  })),
}));
...
describe('DELETE /api/auth/delete-account', () => {
  ...
  it('returns unauthorized when no user is found', async () => {
    supabaseAuth.getUser.mockResolvedValueOnce({ data: { user: null }, error: null });
  ...
  it('returns 500 when admin deletion fails', async () => {
    supabaseAdminClient.auth.admin.deleteUser.mockResolvedValueOnce({ error: new Error('delete failed') });
