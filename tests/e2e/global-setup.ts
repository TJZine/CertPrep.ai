...
export default async function globalSetup(config: FullConfig): Promise<void> {
  ...
  try {
    ...
    if (tokenPart) {
       // Just verify payload is parseable to ensure token integrity
       JSON.parse(Buffer.from(tokenPart.replace(/-/g, '+').replace(/_/g, '/'), 'base64').toString());
    }
  } catch (e) {
  ...
  try {
    ...
    const payload = JSON.parse(Buffer.from(tokenPart.replace(/-/g, '+').replace(/_/g, '/'), 'base64').toString());
