...
export type QuizCore = Pick<Quiz, 'title' | 'description' | 'tags' | 'questions'>;
...
export async function toRemoteQuiz(userId: string, local: Quiz): Promise<RemoteQuizInput> {
  if (local.user_id && local.user_id !== userId) {
    logger.warn('Attempting to sync quiz for mismatched user; skipping user override', { quizId: local.id, quizUserId: local.user_id, userId });
  }
...
export async function toLocalQuiz(remote: RemoteQuizRow): Promise<Quiz> {
  return {
    ...
    deleted_at: remote.deleted_at ? new Date(remote.deleted_at).getTime() : null,
...
export function resolveQuizConflict(
  ...
  const localDeleted = local.deleted_at !== null && local.deleted_at !== undefined;
  ...
  if (remoteDeleted !== localDeleted) {
    return remoteDeleted ? { winner: 'remote', merged: remote } : { winner: 'local', merged: local };
  }
  ...
  if (remoteUpdated > localUpdated) {
    logger.warn('Quiz conflict with equal versions; choosing remote based on updated_at', { quizId: remote.id });
    return { winner: 'remote', merged: remote };
  }

  if (localUpdated > remoteUpdated) {
    logger.warn('Quiz conflict with equal versions; choosing local based on updated_at', { quizId: local.id });
    return { winner: 'local', merged: local };
  }

  logger.warn('Quiz conflict unresolved; defaulting to remote version', { quizId: remote.id });
  return { winner: 'remote', merged: remote };
}
