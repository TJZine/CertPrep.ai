...
export function ImportModal({ isOpen, onClose, onImportSuccess, userId }: ImportModalProps): React.ReactElement | null {
  ...
  const [validationResult, setValidationResult] = React.useState<ValidationResult<QuizImportInput> | null>(null);
  ...
  const validateJson = (text: string): ValidationResult<QuizImportInput> | null => {
  ...
  const handleFileSelect = (file: File): void => {
    ...
    reader.onerror = (): void => addToast('error', 'Failed to read file. Please try again.');
  ...
  const handleImport = async (): Promise<void> => {
    ...
    if (!result?.success || !result.data) {
      const errorMessage = parseError
        ? parseError
        : formatValidationErrors(result?.errors ?? ([] as ValidationError[])) || 'Please fix validation issues.';
    ...
    try {
      ...
      addToast('error', error instanceof Error ? error.message : 'Failed to import quiz');
    } finally {
      setIsImporting(false);
    }
  };

  const handleTabKeyNavigation = (event: React.KeyboardEvent<HTMLDivElement>): void => {
  ...
  const validationStatuses: ValidationStatus[] = React.useMemo(() => {
    ...
    return [
      ...
      { label: `${data.questions.length} questions found`, success: data.questions.length > 0 },
      { label: 'All questions validated', success: true },
    ];
  }, [parseError, validationResult]);

  const formattedErrors = React.useMemo(
    () => (validationResult?.errors ? formatValidationErrors(validationResult.errors) : ''),
    [validationResult],
  );
