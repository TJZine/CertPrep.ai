 ...
 */
export function QuestionReview({ items }: QuestionReviewProps): React.ReactElement {
  return (
    <div className="space-y-4">
      {items.map((item, index) => (
        <div key={item.question} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          ...
          />
          <div className="mt-2 text-sm text-slate-700">Your answer: {item.userAnswer}</div>
          <div className="text-sm text-green-700">Correct answer: {item.correctAnswer}</div>
          {item.explanation ? (
            <div
      ...
      {items.length === 0 ? <div className="text-sm text-slate-500">No review data yet.</div> : null}
    </div>
  );
}
