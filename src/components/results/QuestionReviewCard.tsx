    ...
    >
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex flex-wrap items-center gap-2">
            <span className="font-medium text-slate-500">Q{questionNumber}</span>
          ...
          <p className={cn('text-slate-700', !isExpanded && 'line-clamp-2')}>
            <span
      ...
      {isExpanded && (
        <CardContent className="border-t border-slate-100 pt-4">
          <div className="prose prose-sm prose-slate mb-4 max-w-none" dangerouslySetInnerHTML={{ __html: sanitizedQuestion }} />
            ...
            {sortedOptions.map(([key, text]) => {
              ...
              return (
                <div key={key} className={cn('flex items-start gap-3 rounded-lg border p-3', optionStyle)}>
                  ...
                  <span className="flex-1 text-sm text-slate-700" dangerouslySetInnerHTML={{ __html: sanitizedText }} />
                  {badgeContent}
                </div>
              );
            })}
          ...
          {!userAnswer && (
            <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 p-3">
              <p className="text-sm text-amber-800">You did not answer this question.</p>
            </div>
          )}
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
            ...
            <div className="prose prose-sm prose-slate max-w-none" dangerouslySetInnerHTML={{ __html: sanitizedExplanation }} />
          </div>
