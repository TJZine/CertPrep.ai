  ...
  if (categories.length === 0) {
    return (
      <Card className={className}>
        ...
        <CardContent>
          <p className="text-center text-slate-500">No category data available</p>
        </CardContent>
      </Card>
    );
  }
        ...
        <div className="h-[300px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <RadarChart data={chartData} margin={{ top: 20, right: 30, bottom: 20, left: 30 }}>
              ...
              <PolarAngleAxis dataKey="subject" tick={{ fill: '#64748b', fontSize: 12 }} tickLine={false} />
              <PolarRadiusAxis
                angle={90}
                domain={[0, 100]}
          ...
          {strongest && (
            <div className="text-center">
              <div className="flex items-center justify-center gap-1">
                ...
                <p className="text-sm font-semibold text-green-700">{strongest.score}%</p>
              </div>
              <p className="truncate text-xs text-slate-500" title={strongest.category}>
                {strongest.category}
              </p>
            </div>
          )}
          ...
          {weakest && weakest !== strongest && (
            <div className="text-center">
              <div className="flex items-center justify-center gap-1">
                ...
                <p className="text-sm font-semibold text-red-700">{weakest.score}%</p>
              </div>
              <p className="truncate text-xs text-slate-500" title={weakest.category}>
                {weakest.category}
              </p>
            </div>
          )}
            ...
            <div key={cat.category}>
              ...
              <div className="flex items-center gap-2">
                <div className="h-2 flex-1 overflow-hidden rounded-full bg-slate-200">
                  <div className={cn('h-full transition-all', getScoreColor(cat.score))} style={{ width: `${cat.score}%` }} />
                </div>
