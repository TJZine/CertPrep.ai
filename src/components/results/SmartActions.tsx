...
export function SmartActions({
    ...
    categoryGroups.forEach((questions, category) => {
      ...
      questions.forEach((q, index) => {
        const userAnswerText = q.userAnswer
          ? q.question.options[q.userAnswer]
          : "No answer provided";
        const correctAnswerText = q.correctAnswer
          ? q.question.options[q.correctAnswer]
          : "Unknown";
  ...
  const handleCopyStudyPlan = async (): Promise<void> => {
    ...
    try {
      ...
      addToast(
        "success",
        "Study plan prompt copied! Paste it into ChatGPT, Claude, or your favorite AI.",
      );
  ...
  const handleStartSmartRound = (): void => {
    sessionStorage.setItem(
      SMART_ROUND_QUESTIONS_KEY,
      JSON.stringify(smartRoundQuestionIds),
    );
    sessionStorage.setItem(SMART_ROUND_QUIZ_ID_KEY, quizId);
    sessionStorage.setItem(
      SMART_ROUND_ALL_QUESTIONS_KEY,
      JSON.stringify(allQuestions.map((q) => q.id)),
    );
    sessionStorage.setItem(
      SMART_ROUND_MISSED_COUNT_KEY,
      String(missedQuestions.length),
    );
    sessionStorage.setItem(
      SMART_ROUND_FLAGGED_COUNT_KEY,
      String(flaggedQuestionIds.length),
    );
      ...
      <Modal
        ...
        footer={
          <>
            <Button
              variant="outline"
              onClick={() => setShowSmartRoundModal(false)}
            >
              Cancel
            </Button>
            <Button
              onClick={handleStartSmartRound}
