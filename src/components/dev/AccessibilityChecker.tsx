  ...
  return (
    <div className="fixed bottom-20 right-4 z-50">
      <Card className="w-96 shadow-xl">
        ...
        <CardContent>
          <Button onClick={runCheck} isLoading={isChecking} size="sm" className="mb-4 w-full">
            Run Check
          </Button>
              ...
              {issues.map((issue, index) => (
                ...
                >
                  <div className="flex items-start gap-2">
                    {issue.type === 'error' && <XCircle className="h-4 w-4 flex-shrink-0 text-red-600" />}
                    {issue.type === 'warning' && (
                      <AlertTriangle className="h-4 w-4 flex-shrink-0 text-amber-600" />
                    )}
                    {issue.type === 'info' && <Info className="h-4 w-4 flex-shrink-0 text-blue-600" />}
                    <div>
                      <p className="font-medium">{issue.message}</p>
                      {issue.fix ? <p className="mt-1 text-slate-600">Fix: {issue.fix}</p> : null}
                    </div>
                  </div>
                </div>
              ))}
